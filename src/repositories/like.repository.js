...
async function deleteAllLikes(post_id) {
  const result = await connection.query(
...
async function getUsersLikesPost(post_id) {
  const result = await connection.query(
    `SELECT u.name AS name FROM likes l INNER JOIN users u ON u.id = l.user_id WHERE l.post_id = $1;`,
    [post_id]
  );
  return result;
}
